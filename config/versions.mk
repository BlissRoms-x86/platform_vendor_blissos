...
# Set BLISS version
ifdef BLISS_RELEASE
    BLISS_VERSION := Bliss-OS-v$(VERSION)
else
    BLISS_VERSION := Bliss-OS-$(BLISS_BUILD)-v$(VERSION)-$(TARGET_PRODUCT)-$(BLISS_BUILDTYPE)-$(shell date +%Y%m%d%H%M)
endif

BLISS_DISPLAY_VERSION := $(VERSION)
BLISS_DISPLAY_BUILDTYPE := $(BLISS_BUILDTYPE)
BLISS_FINGERPRINT := Bliss/$(VERSION)/$(TARGET_PRODUCT_SHORT)/$(shell date +%Y%m%d%H%M)
